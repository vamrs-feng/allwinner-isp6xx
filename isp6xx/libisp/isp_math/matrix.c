...
int check_input(float *x, float *y, char* c)
{
	int out = 0;
...
void mat2cvec(int m, int n, float **mat, float *cvec)
{
	/*
	Turns a matrix (given by a float pointer) into its C vector format
...
void mat2fvec(int m, int n, float **mat, float *fvec)
{
	/*
	Turns a matrix (given by a float pointer) into its Fortran vector format
...
void cvec2mat(int m, int n, float* cvec, float** mat)
{
	/*
	Turns a matrix in C vector format into a matrix given by a float pointer
...
void fvec2mat(int m, int n, float *fvec, float **mat)
{
	/*
	Turns a matrix in C vector format into a matrix given by a float pointer
	The matrices have dimension m*n
...
void cvec2fvec(int m, int n, float *cvec, float *fvec)
{
	/*
...
void fvec2cvec(int m, int n, float *fvec, float *cvec)
{
	/*
...
Matrix *create_matrix(int lines, int columns)
{
	...
	// set matrix's value
	a->value = malloc(lines * sizeof(float *));
	if (a->value == NULL) {
		free(a);
		return NULL;
	}
	...
	for (i = 0; i < lines; i++) {
		a->value[i] = calloc(columns, sizeof(float));
		if (a->value == NULL) {
			free(a);
			return NULL;
		}
...
float get_determinant(Matrix *a)
{
	...
	if (a->lines == 1 && a->columns == 1) {
		...
		for (i = 0; i < a->columns; i++) {
			m = get_minor(a, 0, i);
			det += (a->value[0][i]) * (powf(-1, 2 + i)) * (get_determinant(m));
			destroy_matrix(m);
		}
		return det;
	}
...
void multiply_matrix_with_scalar(Matrix *a, float scalar)
{
	int i = 0, j = 0;
...
int compute_inverse(Matrix *a)
{
	...
	for (i = 0; i < transpose->lines; i++) {
		for (j = 0; j < transpose->columns; j++) {
			...
			adjugate->value[i][j] = powf(-1.0, (float)i + (float)j) *
